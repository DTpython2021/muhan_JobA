...
//		}
	...
	public void paint(Graphics window) {
		...
		graphToBack.drawString("블랙잭 게임에 온것을 환영한다 " , 400, 100);
		graphToBack.setColor(Color.BLACK);
		graphToBack.drawString("    딜 러 "  , 25, 270);
		graphToBack.drawString("플레이어 " , 25, 570);
		
		if (deck.size() < 4)
			deck = new Deck(ranks, suits, pointValues);
		if (player.size() > 0)
			graphToBack.drawString("플레이어 점수: " + player.score(), 1000, 550);
		if (player.score() == 21 || dealer.score() == 21)
			endCheck();
		if (message != "") {
			graphToBack.drawString("딜러 점수: " + dealer.score(), 1000, 250);
